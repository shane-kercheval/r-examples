...
```{r}
quantile(wells$arsenic, c(0, 0.35, .5, 1))
```
...
While this might be faster, at a glance it is hard to tell what is going on, and the code isn't explained.

We can just use `predict()` to get the same thing, which I already wrapped in a helper function, `predict_switch`, above.

Also, they are using an arsenic level of `0.5` which is actually below (although slightly) the minimum arsenic level in the data, and comparing that to an arsenic value of 1, which is less than the median. Seems like weird and arbitrary references. I'll use min/medium/max values.
