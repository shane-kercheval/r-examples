...
```{r chapter_13_mcmc_areas_coefficient_density, fig.height=6, fig.width=plot_width_height_6, message=FALSE}
bayesplot::mcmc_areas(as.matrix(model_1)) +
                      #regex_pars = "^log|^gro") +
    labs(title = "Coefficient Densities")
```

```{r chapter_13_logistic_coef_intervals, fig.height=6, fig.width=plot_width_height_6, message=FALSE}
...
> A Poisson process is a special type of **counting** process. Given a stream of events that arrive at random times starting at `t=0`, let `Nt` denote the `number of arrivals` that occur by time `t`, that is, the number of events in `[0, t]`. For instance, `Nt` might be the number of text messages received up to time `t`. (stochastic processes pg 223)

> The distribution of the number o farrivals/events in an interval only depends on the length of the interval. (pg 225)

> `E(Nt) = lambda*t`; That is, we expect about `lambda * t` arrivals/events in `t` time unites. Thus, **the `rate` of arrival is `E(Nt) / t = lambda`**. (pg)
...
```{r chapter_15_poisson_distribution, fig.height=5, fig.width=plot_width_height_5}
...
Note below: https://www.healthknowledge.org.uk/public-health-textbook/research-methods/1b-statistical-methods/statistical-distributions#:~:text=Binomial%20distribution%20describes%20the%20distribution,r%20events%20in%20a%20population 

> The `Binomial` distribution describes the distribution of binary data from a **finite** sample. Thus it gives the `probability` of getting `r` events out of `n` trials.

> The `Poisson` distribution describes the distribution of binary data from an **infinite** sample. Thus it gives the `probability` of getting `r` events in a `population`.
...
```{r chapter_15_poisson_fitted_simulated_actual, fig.height=6, fig.width=plot_width_height_6}
fake %>%
...
```{r include=FALSE}
rm(poisson_random_values)
rm(poisson_expected_density)
rm(x_range)
rm(lambda)
rm(x_actual)
rm(y_actual)
...
rm(fake)
rm(model_fake)
rm(fake_model_simulation)
```

# Chapter 17 - Poststratification and Missing-Data Imputation

## Poststratification - Election Polls

> Example from https://avehtari.github.io/ROS-Examples/Poststrat/poststrat.html
...
set.seed(2)
poll[sample(x=1:nrow(poll), size=10),]
```
...
Note, some votes can be `NA`. There are no comments or descriptions in the code, but from what I can tell, this is their way of creating the imbalance between the population and the survey.

If we look at the entire dataset, we see the expected population proportions of `~36%` Democrate, `~31%` Independent, `~33%` Republican.

The percent of trump votes for each group matches 

```{r}
population_summary <- poll %>%
    group_by(pid) %>%
    summarise(num_trump_votes = sum(vote, na.rm = TRUE),
              percent_trump_votes = mean(vote, na.rm = TRUE),
              #group_size = n(),
              group_percent = n() / nrow(.)) %>%
    ungroup()
population_summary
```

If we look at the data without `NAs`, we get the sample of data that represents the survey. 

> Of these respondents, Party ID `33%` Republican, `40%` Democrat, `27%` independent.

Note that the percent of trump votes for each party identification group remains the same across population (above) and survey (below).

> More precisel, we make the assumption that the probability of a survey reponsdent supporting Trump (...) conditional on party identification, is the same as the corresponding probability among the poopulation of voters. (pg 314)

Note: that the regression model will automatically remove `NA` values and so will use the dataset below. I don't think this is mentioned by authors, which creates additional confusion in following their code.
...
Current Percent of Trump Votes in survey:

```{r}
sum(survey_summary$num_trump_votes) / nrow(poll %>% filter(!is.na(vote)))
```

We get the same thing by weighted average of percent_trump_votes

```{r}
sum(survey_summary$percent_trump_votes * survey_summary$group_percent)
```

We can do a simple post-stratifcation by using the true group_percent rather than the survey group percent

```{r}
sum(survey_summary$percent_trump_votes * c(.36, 0.31, 0.33))
```
...
sum(poststrat_data$N * y_bar_cells)
```
...
```{r}
# note, stan_glm will automatically filter out NAs
set.seed(1)
fit_1 <- stan_glm(vote ~ pid, data = poll, refresh = 0)
...
apparently we can have a column `N` in our `newdata` and `posterior_epred` knows what to do with it.
...
```{r}
poststrat_est_1 <- epred_1 %*% poststrat_data$N
head(poststrat_est_1)
```

```{r}
c(mean(poststrat_est_1), mad(poststrat_est_1))
```
...
```{r}
pid_weights <- population_summary$group_percent / survey_summary$group_percent
names(pid_weights) <-  survey_summary$pid
pid_weights
```

---

I thought i could use `weights`, but this gives the same answer, because we are already controlling for the thing we are basing the weights off of.. .doesn't make any sense to use weights here..

```{r}
poll_weights <- poll %>%
    filter(!is.na(vote)) %>%
    mutate(weight = pid_weights[pid])

poll_weights %>% distinct()
```

```{r}
# note, stan_glm will automatically filter out NAs
set.seed(1)
fit_weights <- stan_glm(vote ~ pid, data = poll_weights, weights = weight, refresh = 0)
print(fit_weights, digits=3)
```

Same exact model as `fit_1`

```{r}
epred_weights <- posterior_epred(fit_weights, newdata=survey_summary %>% select(pid))
head(epred_weights)

colMeans(epred_weights)
```

```{r include=FALSE}
rm(population_summary)
rm(survey_summary)
rm(poststrat_data)
rm(fit_1)
rm(epred_1)
