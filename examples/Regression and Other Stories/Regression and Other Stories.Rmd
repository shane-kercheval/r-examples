...
> We fitted the model as `Pr(y_i = 1) = logit^-1(-1.4 + 0.3 * income) (p.g 218)

We can get the probabilities i.e. expected outcome via `posterior_epred`:

```{r}
new_data <- data.frame(income=5)
head(posterior_epred(object=model_1, newdata = new_data))
```

**We can use this type of prediction (above) to make statements about uncertainty about the population (pg 224)**

Alternatively, we can get the binary prediction (`0` or `1`) via `posterior_predict`:

```{r}
head(posterior_predict(object=model_1, newdata = new_data))
```

**We can use this type of prediction (above) to make statements about uncertainty about individual people (pg 224).**

Or, prediction given a range of input values:

```{r}
new_data <- data.frame(income=1:5)

expected_predictions <- posterior_epred(model_1, newdata=new_data)
head(expected_predictions)
```

```{r}
apply(expected_predictions, 2, function(.x) quantile(.x, c(0, .5, 1)))
```

```{r}
colMeans(expected_predictions)
```



```{r}
binary_predictions <- posterior_predict(model_1, newdata=new_data, type='link')
head(binary_predictions)
```

```{r}
apply(binary_predictions, 2, function(.x) quantile(.x, c(0, .5, 1)))
```

```{r}
colMeans(binary_predictions)
```


NOTE explain posterior_epred vs posterior_predict.. The below graph is making statemtns about the pouplation. posterior_predict.. is making statements about individuals

```{r chapter_12_nes_1992_predictions, fig.height=6, fig.width=plot_width_height_6, message=FALSE}
expected_predictions <- expected_predictions %>%
    as.data.frame() %>%
    pivot_longer_all(names_to='income', values_to='prediction') %>%
    mutate(income = as.numeric(income))

expected_predictions %>%
    ggplot(aes(x=income, y=prediction, color=NULL)) +
    geom_jitter(width=0.1, alpha=0.1) +
    geom_hline(yintercept = 0.5, color='red') +
    geom_vline(xintercept = xmin$minimum, color='red') +
    #geom_errorbar(aes(ymin=conf.low.68, ymax=conf.high.68), width=0.5) +
    #geom_errorbar(aes(ymin=conf.low.95, ymax=conf.high.95), width=0.7) +
    geom_line(data=data.frame(x=seq(-6, 12, by=0.1)) %>%
    ...
    #scale_color_manual(values=c("#7A7A7A", "#7AA9CF")) +
    labs(title="Prediction of Probability that Vote is Republican, given Income.",
...
rm(model_1)
rm(new_data)
rm(expected_predictions)
rm(binary_predictions)
```
