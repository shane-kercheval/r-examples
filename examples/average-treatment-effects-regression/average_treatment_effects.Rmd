...
Before, the `selected` coefficient was the predicted difference in `selected` vs `not selected` across all `numhh` values. Now, however, we've added an interaction term and, as a result, the difference between a `selected` vs `not selected` with the same `numhh` depends on what the `numhh` is. **So the coefficient for `selected` is the predicted difference between a `selected` vs `not selected` person who both have `numhh1` (1 person in the household), since that is the reference/holdout group.**
