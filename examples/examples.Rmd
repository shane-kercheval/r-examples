...
filter_over_partition_by <- function(.x, .col_name, .partition_by, ...) {
    .x %>%
        mutate(temp______original_order = row_number()) %>%
        group_by(across({{ .partition_by }})) %>%
        arrange( ... , .by_group = TRUE) %>%
...
```{r}
row_number_over_partition_by <- function(data, ...) {
    data %>% arrange( ... )
}
mtcars %>% 
    select(cyl, am, vs, mpg) %>%
    row_number_over_partition_by(desc(vs), mpg)
```


```{r}
mtcars %>% arrange_at(c("desc(cyl)", "mpg"))
```



```{r}
mtcars %>% arrange(across(quos(c(mpg, cyl))))
mtcars %>% arrange(quos(c(mpg, cyl)))

mtcars %>% arrange(quos(c(mpg, cyl)))
mtcars %>% arrange(!!quos(c(mpg, cyl)))

mtcars %>% arrange(!!! rlang::syms(c("mpg", "cyl")))

vector_of_vars <- c("desc(mpg)", "cyl")
mtcars %>% arrange_(.dots=vector_of_vars)
