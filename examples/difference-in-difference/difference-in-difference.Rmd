...
sem_per_dma <- sem %>%
    ...
    # author takes mean(log) but i'm not sure why we won't take lo
    summarise(y = mean(log(revenue)), n=n()) %>%
    #summarise(y = log(sum(revenue)), n=n()) %>%
    ungroup() %>%
...
```{r}
coefficients(reg_results)['treatment_period:variantTreatment'] * 100
```

original/regression standard errors

```{r}
summary(reg_results)$coef['treatment_period:variantTreatment',]
```
...
```{r}
library(AER)
(standard_error <- sqrt(vcovCL(reg_results, cluster=sem_per_dma$dma)['treatment_period:variantTreatment','treatment_period:variantTreatment']))
```

```{r}
coefficients(reg_results)['treatment_period:variantTreatment'] + (c(-2, 2) * standard_error)
```



The author uses the average of log-revenue, i'm not sure why we wouldn't want to sum revenue across dma then take log
